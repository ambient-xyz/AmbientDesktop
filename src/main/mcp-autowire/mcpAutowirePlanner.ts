...
type McpAutowireNetworkConfidence = "low" | "medium" | "high";

interface McpAutowireNetworkHostFact {
  host: string;
  ports: number[];
  purpose: string;
...
interface McpAutowireNetworkFacts {
...
function classificationFetchPreview(entry: McpAutowireDiscoveryFetch): string {
...
async function resolveMcpAutowireNetworkFacts(input: {
  input: McpAutowirePlanInput;
  options: McpAutowirePlannerOptions;
  target: McpAutowireTarget;
  session: McpAutowireSessionFacts;
  ...
  const evidenceText = deterministicEvidenceText(evidenceInput);
  const packageIdentity = deterministicPackageIdentity(evidenceInput, evidenceText);
  if (!packageIdentity) return undefined;
  const deterministic = deterministicMcpAutowireNetworkFacts(evidenceInput);
  ...
  try {
    return await callWorkflowPiJson<McpAutowireNetworkFacts>({
      ...
      prompt: mcpAutowireNetworkRequirementsPrompt({
        ...
        packageIdentity,
        deterministic,
        evidenceEntries: mcpAutowireNetworkEvidenceEntries(evidenceInput),
      }),
...
function mcpAutowireNetworkRequirementsPrompt(input: {
  targetUrl: string;
  packageIdentity: DeterministicPackageIdentity;
...
function validateMcpAutowireNetworkFacts(value: unknown, fallback: McpAutowireNetworkFacts): McpAutowireNetworkFacts {
  ...
  const nonRuntimeHosts = Array.isArray(value.nonRuntimeHosts)
    ...
    : [];
  const emptyRuntimeOverridesFallback = fallback.runtimeHosts.length > 0 &&
    runtimeHosts.length === 0 &&
    nonRuntimeHostsCoverFallback(nonRuntimeHosts, fallback.runtimeHosts);
  if (fallback.runtimeHosts.length && !runtimeHosts.length && !emptyRuntimeOverridesFallback) {
    throw new Error("runtimeHosts cannot be empty when deterministic evidence found host candidates unless nonRuntimeHosts explains each candidate");
  }
  const openQuestions = Array.isArray(value.openQuestions)
    ? value.openQuestions.filter((question): question is string => typeof question === "string" && Boolean(question.trim())).map((question) => question.trim()).slice(0, 6)
    : [];
  return {
...
function normalizeNetworkHostFact(value: unknown): McpAutowireNetworkHostFact[] {
  ...
  if (!host) return [];
  const ports = uniqueSortedNumbers(Array.isArray(value.ports) ? value.ports.filter((port): port is number => Number.isInteger(port) && port > 0 && port <= 65535) : [443]);
...
function deterministicMcpAutowireNetworkFacts(input: {
  target: McpAutowireTarget;
  instructions?: string;
  discoverySummary: string;
  fetches: McpAutowireDiscoveryFetch[];
  searches: McpAutowireDiscoverySearch[];
}): McpAutowireNetworkFacts {
  ...
  const evidenceText = mcpAutowireNetworkEvidenceEntries(input).map((entry) => entry.text).join("\n");
  const needsBroadNetwork = !runtimeHosts.length && /\b(?:api|http|https|fetch\(|axios|requests|httpx|aiohttp|urllib|websocket)\b/i.test(evidenceText);
  return {
    runtimeHosts,
    nonRuntimeHosts: [],
    needsBroadNetwork,
...
function mcpAutowireNetworkEvidenceEntries(input: {
  target: McpAutowireTarget;
  instructions?: string;
  discoverySummary: string;
  fetches: McpAutowireDiscoveryFetch[];
  searches: McpAutowireDiscoverySearch[];
}): McpAutowireNetworkEvidenceEntry[] {
  return [
    ...
    ...input.searches.flatMap((search) => (search.results ?? []).map((result) => ({
...
function runtimeHostScore(host: string, context: string, locator: string): number {
  ...
  let score = 4;
  if (/\b(api|endpoint|base url|baseurl|powered by|provider|service|fetch|request|requests|httpx|axios|client|cep|holiday|holidays)\b/.test(text)) score += 10;
...
function bestNetworkEvidenceLocator(facts: McpAutowireNetworkFacts): string | undefined {
  return facts.runtimeHosts.flatMap((fact) => fact.evidence.map((entry) => entry.locator)).find(Boolean);
}

function networkJustification(facts: McpAutowireNetworkFacts, packageName: string): string {
...
function normalizeAnyHostname(value: string): string | undefined {
  const trimmed = value.trim().toLowerCase().replace(/^https?:\/\//, "").replace(/[/?:#].*$/, "").replace(/^\.+|\.+$/g, "");
...
function isIgnoredRuntimeHost(host: string): boolean {
  return ignoredRuntimeHostSuffixes.some((suffix) => host === suffix || host.endsWith(`.${suffix}`)) ||
    ignoredRuntimeHostExact.has(host);
}
...
const ignoredRuntimeHostExact = new Set([
