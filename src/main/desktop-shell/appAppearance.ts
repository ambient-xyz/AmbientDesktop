...
import type { LocalDeepResearchSettings, MediaPlaybackSettings, SttSettings, VoiceSettings } from "../../shared/localRuntimeTypes";
...
export interface AppAppearanceProviderPreferenceNormalizers {
  defaultSearchRoutingSettings: SearchRoutingSettings;
  defaultLocalDeepResearchSettings: LocalDeepResearchSettings;
  normalizeSearchRoutingSettings(value: unknown): SearchRoutingSettings;
  normalizeLocalDeepResearchSettings(value: unknown): LocalDeepResearchSettings;
}

export interface AppAppearanceProviderPreferences {
  DEFAULT_SEARCH_ROUTING_SETTINGS: SearchRoutingSettings;
  DEFAULT_LOCAL_DEEP_RESEARCH_SETTINGS: LocalDeepResearchSettings;
  normalizeSearchRoutingSettings(value: unknown): SearchRoutingSettings;
  normalizeLocalDeepResearchAppSettings(value: unknown): LocalDeepResearchSettings;
  readSearchRoutingSettings(filePath: string): Promise<SearchRoutingSettings>;
  readLocalDeepResearchSettings(filePath: string): Promise<LocalDeepResearchSettings>;
  writeSearchRoutingSettings(filePath: string, search: SearchRoutingSettings): Promise<void>;
  writeLocalDeepResearchSettings(filePath: string, localDeepResearch: LocalDeepResearchSettings): Promise<void>;
}
...
export function createAppAppearanceProviderPreferences(
  normalizers: AppAppearanceProviderPreferenceNormalizers,
): AppAppearanceProviderPreferences {
  function normalizeSearchRoutingSettings(value: unknown): SearchRoutingSettings {
    return normalizers.normalizeSearchRoutingSettings(value);
  }

  function normalizeLocalDeepResearchAppSettings(value: unknown): LocalDeepResearchSettings {
    return normalizers.normalizeLocalDeepResearchSettings(value);
  }

  async function readSearchRoutingSettings(filePath: string): Promise<SearchRoutingSettings> {
    try {
      const parsed = JSON.parse(await readFile(filePath, "utf8")) as Record<string, unknown>;
      return normalizeSearchRoutingSettings(parsed.search);
    } catch {
      return normalizers.defaultSearchRoutingSettings;
    }
  }

  async function readLocalDeepResearchSettings(filePath: string): Promise<LocalDeepResearchSettings> {
    try {
      const parsed = JSON.parse(await readFile(filePath, "utf8")) as Record<string, unknown>;
      return normalizeLocalDeepResearchAppSettings(parsed.localDeepResearch);
    } catch {
      return normalizers.defaultLocalDeepResearchSettings;
    }
  }

  async function writeSearchRoutingSettings(filePath: string, search: SearchRoutingSettings): Promise<void> {
    let existing: Record<string, unknown> = {};
    try {
      existing = JSON.parse(await readFile(filePath, "utf8")) as Record<string, unknown>;
    } catch {
  ...
  async function writeLocalDeepResearchSettings(
    filePath: string,
    ...
    await writeFile(
      filePath,
      `${JSON.stringify({ ...existing, localDeepResearch: normalizeLocalDeepResearchAppSettings(localDeepResearch) }, null, 2)}\n`,
      "utf8",
    );
  }

  return {
    DEFAULT_SEARCH_ROUTING_SETTINGS: normalizers.defaultSearchRoutingSettings,
    DEFAULT_LOCAL_DEEP_RESEARCH_SETTINGS: normalizers.defaultLocalDeepResearchSettings,
