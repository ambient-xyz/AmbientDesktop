...
import { browserRuntimeForAgentProfile, browserToolFallback, browserUnavailableText, selectAgentBrowserRuntime } from "./agentBrowserRuntime";
import {
  AgentRuntime,
...
import { providerCatalogBootstrapReminder } from "./providerCatalog";
...
describe("AgentRuntime sub-agent local runtime routing", () => {
  ...
  it("round-trips child browser authority prompts through parent approval and child resume", async () => {
    ...
    try {
      ...
      const spawned = await subagentTool.execute("spawn-browser-approval-roundtrip", {
        action: "spawn_agent",
        roleId: "explorer",
        ...
        promptMode: "fresh",
        toolScope: { requestedCategories: ["browser.read"] },
      ...
      const waited = await subagentTool.execute("wait-browser-approval-roundtrip", {
        action: "wait_agent",
        childRunId: run.id,
        wait: { timeoutMs: 5000 },
      ...
      const resumed = await subagentTool.execute("wait-browser-approval-response-roundtrip", {
        action: "wait_agent",
        childRunId: run.id,
        wait: { timeoutMs: 1 },
      ...
      expect(pendingPermissions).toEqual([]);
      await expect(browserPermissionPromise).resolves.toBeUndefined();
      expect(resumed.details).toMatchObject({
        status: "running",
        waitSatisfied: false,
        waitTimedOut: true,
...
describe("Planner Mode system prompt", () => {
  ...
  it("completes the source artifact when planner finalization creates a durable artifact", async () => {
    ...
    try {
      ...
      const runtime = new AgentRuntime(store, { refreshWorkspaceArtifact: vi.fn(async () => undefined) } as any, {} as any, () => undefined, {
        request: vi.fn(),
        denyThread: () => undefined,
      }, {
        planner: {
      ...
      const result = await (runtime as any).createPlannerPlanArtifactFromMessage(finalMessage);
...
describe("AgentRuntime Local Deep Research tools", () => {
  ...
  it("surfaces active sub-agent local runtime leases in Local Deep Research setup", async () => {
    ...
    try {
      ...
      expect(result.content[0].text).toContain("Local runtime inventory: 1 runtime; 1 active lease; In use by sub-agent Review worker.");
      expect(result.details.localRuntimeInventory).toMatchObject({
        activeLeases: [
          expect.objectContaining({
            leaseId: "lease-review",
            parentThreadId: "parent-thread",
...
describe("AgentRuntime active-run queue activity", () => {
  it("does not treat stale send permission mode as a runtime thread setting update", () => {
  ...
  it("does not downgrade the active thread permission mode when queuing a stale follow-up", async () => {
    ...
    const store = {
      ...
      addMessage: vi.fn((message: any) => ({ id: "message-queued", createdAt: new Date().toISOString(), ...message })),
      markThreadRead: vi.fn(() => thread),
      getWorkspace: vi.fn(() => ({ statePath: "/tmp/ambient-test-state" })),
    };
  ...
  it("forwards queued active-run activity to awaiting callers", async () => {
    const thread = {
      id: "thread-queued-activity",
      title: "queued activity",
      permissionMode: "workspace",
      collaborationMode: "agent",
      model: "ambient-preview",
      thinkingLevel: "medium",
...
describeNative("AgentRuntime first-party plugin permissions", () => {
  ...
  it("recovers visible transcript when no Pi session file was recorded", async () => {
    const workspacePath = await mkdtemp(join(tmpdir(), "ambient-runtime-context-recover-no-session-"));
    const store = new ProjectStore();
    try {
      const workspace = store.openWorkspace(workspacePath);
      ...
      const snapshot = await runtime.recoverThreadContext({ threadId: thread.id });
  ...
  it("recovers visible transcript when the recorded Pi session file is missing", async () => {
    const workspacePath = await mkdtemp(join(tmpdir(), "ambient-runtime-context-recover-missing-session-"));
    const store = new ProjectStore();
    try {
      const workspace = store.openWorkspace(workspacePath);
      ...
      await runtime.recoverThreadContext({ threadId: thread.id });
  ...
  it("rejects recovery when there is no visible transcript to rebuild from", async () => {
    const workspacePath = await mkdtemp(join(tmpdir(), "ambient-runtime-context-recover-empty-"));
    const store = new ProjectStore();
    try {
      store.openWorkspace(workspacePath);
      ...
      await expect(runtime.recoverThreadContext({ threadId: thread.id })).rejects.toThrow(
  ...
  it("rejects lossy recovery when the existing Pi session file is restorable", async () => {
    const workspacePath = await mkdtemp(join(tmpdir(), "ambient-runtime-context-recover-restorable-"));
    const store = new ProjectStore();
    try {
      const workspace = store.openWorkspace(workspacePath);
  ...
  it("does not duplicate recovery messages when recovery has already produced a restorable session", async () => {
    const workspacePath = await mkdtemp(join(tmpdir(), "ambient-runtime-context-recover-idempotent-"));
    const store = new ProjectStore();
    try {
      const workspace = store.openWorkspace(workspacePath);
