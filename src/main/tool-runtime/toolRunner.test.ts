...
describe("normalizeToolRunnerRuntimeEnv", () => {
  it("prefers native Homebrew Node for Pi bash on Apple Silicon", () => {
    const env = normalizeToolRunnerRuntimeEnv(
      {
        PATH: ["/Users/Neo/.pi/agent/bin", "/usr/local/bin", "/opt/homebrew/bin", "/usr/bin"].join(delimiter),
        HOME: "/Users/Neo",
      },
    ...
    expect(env.PATH?.split(delimiter).slice(0, 4)).toEqual(["/opt/homebrew/bin", "/opt/homebrew/sbin", "/Users/Neo/.pi/agent/bin", "/usr/local/bin"]);
  ...
  it("does not force native Node for plugin MCP processes or non-arm64 hosts", () => {
    const base = { PATH: "/Users/Neo/.pi/agent/bin:/usr/local/bin" };
    const host = {
