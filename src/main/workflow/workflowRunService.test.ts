...
import type { MiniCpmVisionAnalysisResult, MiniCpmVisionAnalyzeInput } from "../../shared/localRuntimeTypes";
...
import { resolveWorkflowApproval } from "./workflowDashboard";
...
function fixtureAutomationPluginRegistry(
  availability: "available" | "untrusted" | "auth-required" | "disabled" = "available",
...
`,
    ...
    const analyzeMiniCpm = vi.fn(
      async (_workspacePath: string, input: MiniCpmVisionAnalyzeInput): Promise<MiniCpmVisionAnalysisResult> => ({
        provider: "minicpm-v",
        status: "passed",
        ...
        limitations: [],
        image: {
          path: input.image?.path ?? input.imagePath ?? "screenshots/main.png",
          basename: "main.png",
          bytes: 10,
          sha256: "a".repeat(64),
    ...
    const state = JSON.parse(await readFile(artifact.statePath, "utf8")) as {
      checkpoints?: Record<string, { value?: { summary?: string } }>;
    };
    ...
    expect(events.map((event) => `${event.type}:${event.message}`)).toEqual(
      expect.arrayContaining(["desktop-tool.end:ambient_visual_analyze"]),
    );
...
`,
    ...
    const events = store.listWorkflowRunEvents(dashboard.runs[0].id);
    expect(events.map((event) => event.type)).toEqual(
      expect.arrayContaining(["ambient.call.start", "workflow.timeout", "workflow.paused"]),
    );
    expect(events.map((event) => event.type)).not.toContain("workflow.failed");
    expect(events.find((event) => event.type === "workflow.timeout")).toMatchObject({
      data: expect.objectContaining({
