...
export async function acquireSymphonyMutationWorkspaceLease(
  ...
  try {
    lease = activeWorktree
      ...
      : await scratchOverlayLease(input, now);
...
async function scratchOverlayLease(
  ...
  const isGit = await (input.isGitWorkspace ?? defaultIsGitWorkspace)(input.parentThread.workspacePath);
  if (isGit) {
...
async function defaultIsGitWorkspace(workspacePath: string): Promise<boolean> {
