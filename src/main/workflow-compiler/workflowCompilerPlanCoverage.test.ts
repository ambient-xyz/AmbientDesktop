...
import { compileWorkflowProgramIr, type WorkflowProgramAmbientCliCapability } from "./workflowCompilerWorkflowProgramFacade";
...
describe("fixWorkflowCompiler deterministic plan coverage", () => {
  ...
  it("case 6: compiles bounded bash test execution and Ambient failure classification without raw process access", async () => {
    const result = await compilePlanCase({
      ...
      nodes: [
        { id: "run-tests", kind: "tool.call", tool: "bash", args: { command: "pnpm exec vitest run src/main/workflow-program/workflowProgramCompiler.test.ts --reporter=dot" } },
        {
          id: "classify-failure",
          kind: "model.call",
          dependsOn: ["run-tests"],
