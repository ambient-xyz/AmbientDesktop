...
describe("subagentCompletionGuard", () => {
  ...
  it("accepts Ambient mutation evidence as isolated when the run owns a mutation workspace lease", () => {
    expect(validateSubagentCompletionGuard({
      role: getDefaultSubagentRoleProfile("worker"),
      run: run({
        roleId: "worker",
        ...
        resultArtifact: resultArtifact({
          mutationEvidence: [{ toolCallId: "tool-call-1", path: "generated/app.ts", category: "workspace.write" }],
        }),
      }),
      events: [
        event({
  ...
  it("does not treat mutation lease ownership as isolation for events outside lease writable roots", () => {
    expect(validateSubagentCompletionGuard({
      role: getDefaultSubagentRoleProfile("worker"),
      run: run({
        roleId: "worker",
        symphonyMutationWorkspaceLease: mutationWorkspaceLease({ status: "released" }),
        resultArtifact: resultArtifact({
          mutationEvidence: [{ toolCallId: "tool-call-1", path: "/repo/unsafe.ts", category: "workspace.write" }],
        }),
      }),
      events: [
        event({
...
function mutationWorkspaceLease(
  ...
  return {
    ...
    leaseId: "symphony-mutation:worker-run",
    parentThreadId: "parent-thread",
    childThreadId: "child-thread",
    childRunId: "worker-run",
    kind: "scratch_overlay",
    rootPath: "/tmp/ambient-symphony-mutation-leases/worker-run",
    sourceRoots: ["/repo"],
    readOnlyBaseRoots: ["/repo"],
    declaredWritableRoots: ["/repo/generated"],
    writableRoots: ["/tmp/ambient-symphony-mutation-leases/worker-run/generated"],
    status: "active",
    acquiredAt: "2026-06-05T00:00:01.000Z",
    lastHeartbeatAt: "2026-06-05T00:00:02.000Z",
    ...overrides,
  };
}
