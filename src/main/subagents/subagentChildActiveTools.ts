...
export function resolveSubagentChildActiveToolActivation(input: {
  ...
  const deniedBuiltInToolNames = new Set(snapshot.scope.deniedTools
    .filter((tool) => tool.source === "built_in")
    .map((tool) => tool.id));
  ...
  return {
    activeToolNames: dedupeToolNames(activeToolNames)
      .filter((toolName) => !CHILD_NEVER_INHERIT_TOOL_NAMES.has(toolName) && !deniedBuiltInToolNames.has(toolName)),
    unavailableExtensionToolNames,
    unavailableCallableWorkflowToolNames,
  };
}
