...
async function readAgentMemoryDiagnostics(store: DiagnosticDataSource): Promise<DiagnosticAgentMemoryState> {
  const errors: string[] = [];
  if (!store.getAgentMemoryDiagnostics) return { errors };
  try {
    return { diagnostics: await store.getAgentMemoryDiagnostics(), errors };
  } catch (error) {
    return { errors: [`Agent memory diagnostics failed: ${errorMessage(error)}`] };
  }
}
