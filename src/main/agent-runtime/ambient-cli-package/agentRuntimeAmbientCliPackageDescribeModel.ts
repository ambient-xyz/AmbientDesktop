...
export function ambientCliDescribeText(result: AmbientCliPackageDescription): string {
  ...
  for (const command of result.commands) {
    lines.push(
      ...
      command.timeoutProfile ? `Timeout profile: ${command.timeoutProfile}` : undefined,
      command.progressPatterns?.length ? `Progress patterns: ${command.progressPatterns.join("; ")}` : undefined,
      command.devicePolicy ? `Device policy: ${JSON.stringify(command.devicePolicy)}` : undefined,
