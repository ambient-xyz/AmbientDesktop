...
describe("slash command catalog", () => {
    ...
    const entries = buildSlashCommandCatalogEntries({
      ...
      callableWorkflowRegistry: callableWorkflowRegistry({
        entries: [callableWorkflowCatalogEntry({
          id: "symphony:fanout:v1",
          label: "Symphony Fanout",
          summary: "Runs a parent-visible Symphony recipe.",
          sourceKind: "symphony_recipe",
          sourceId: "symphony-fanout",
          toolName: "symphony_fanout",
        })],
      }),
    });
  ...
  it("keeps disabled and archived recorded callable workflows unavailable", () => {
    ...
    const response = buildSlashCommandSearchResponse({
      ...
      callableWorkflowRegistry: callableWorkflowRegistry({
        entries: [
          recordedCallableWorkflowEntry(workflowRecordingDescription({ id: "ready", title: "Ready Flow" })),
          recordedCallableWorkflowEntry(workflowRecordingDescription({ id: "disabled", title: "Disabled Flow", enabled: false }), {
            status: "excluded_not_callable",
            exclusionReasons: ["recorded_workflow_disabled"],
          }),
          recordedCallableWorkflowEntry(workflowRecordingDescription({
            id: "archived",
            title: "Archived Flow",
...
interface CallableWorkflowCatalogEntryFixture {
  id: string;
  label: string;
  summary: string;
  sourceKind: string;
...
function callableWorkflowRegistry(input: {
  featureFlagEnabled?: boolean;
  entries: CallableWorkflowCatalogEntryFixture[];
}) {
  return {
    catalogStatus: {
...
function callableWorkflowCatalogEntry(
  overrides: Partial<CallableWorkflowCatalogEntryFixture> = {},
): CallableWorkflowCatalogEntryFixture {
  return {
    id: overrides.id ?? "symphony:default:v1",
    label: overrides.label ?? "Symphony Recipe",
    summary: overrides.summary ?? "Runs a callable recipe.",
    sourceKind: overrides.sourceKind ?? "symphony_recipe",
    sourceId: overrides.sourceId ?? "symphony-default",
    sourceVersion: overrides.sourceVersion ?? "v1",
    status: overrides.status ?? "ready",
    parentPiVisible: overrides.parentPiVisible ?? true,
    toolName: overrides.toolName,
    inputSchemaRequired: overrides.inputSchemaRequired ?? [],
    exclusionReasons: overrides.exclusionReasons ?? [],
  };
}

function recordedCallableWorkflowEntry(
  recording: WorkflowRecordingLibraryDescription,
  overrides: Partial<CallableWorkflowCatalogEntryFixture> = {},
): CallableWorkflowCatalogEntryFixture {
  return callableWorkflowCatalogEntry({
    id: `recorded:${recording.id}:v${recording.version}`,
    label: recording.title,
    summary: recording.summary,
    sourceKind: "recorded_workflow",
    sourceId: recording.id,
    sourceVersion: recording.version,
    toolName: `workflow_${recording.id}`,
    ...overrides,
  });
}
