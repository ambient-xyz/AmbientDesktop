...
import {
  ProjectStoreProjectBoardSourceImpactRepository,
...
export class ProjectStoreProjectBoardSourceRepository {
  ...
  replaceProjectBoardSources(boardId: string, sources: ProjectBoardSourceInput[]): ProjectBoardSource[] {
    ...
    const { preservedClassificationCount, newCount, changedCount, unchangedCount, removedCount } = refreshStats;
    const transaction = this.db.transaction(() => {
      ...
      if (!board.source_thread_id && inferredSourceThreadId) {
        this.db
          .prepare("UPDATE project_boards SET source_thread_id = ?, updated_at = ? WHERE id = ?")
          .run(inferredSourceThreadId, now, boardId);
      } else {
  ...
  getProjectBoardSource(sourceId: string): ProjectBoardSource {
    const row = this.db.prepare("SELECT * FROM project_board_sources WHERE id = ?").get(sourceId) as ProjectBoardSourceStoreRow | undefined;
  ...
  updateProjectBoardSource(input: { sourceId: string; kind: ProjectBoardSourceKind; includeInSynthesis?: boolean }): ProjectBoardSource {
    const current = this.db.prepare("SELECT * FROM project_board_sources WHERE id = ?").get(input.sourceId) as
    ...
    const row = this.db.prepare("SELECT * FROM project_board_sources WHERE id = ?").get(input.sourceId) as
      | ProjectBoardSourceStoreRow
      | undefined;
  ...
  stageProjectBoardSourceDraftPiUpdates(input: StageProjectBoardSourceDraftPiUpdatesInput): ProjectBoardSummary {
    return this.sourceImpact().stageProjectBoardSourceDraftPiUpdates(input);
  }

  refreshProjectBoardSourceDrafts(input: RefreshProjectBoardSourceDraftsInput): ProjectBoardSummary {
    return this.sourceImpact().refreshProjectBoardSourceDrafts(input);
  }

  applyProjectBoardSourceImpactFeedback(input: ApplyProjectBoardSourceImpactFeedbackInput): ProjectBoardSummary {
    return this.sourceImpact().applyProjectBoardSourceImpactFeedback(input);
  }

  private sourceImpact(): ProjectStoreProjectBoardSourceImpactRepository {
    return new ProjectStoreProjectBoardSourceImpactRepository(this.db, this.deps);
  }
