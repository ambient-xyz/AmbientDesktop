...
describe("ProjectStoreSubagentRunRepository", () => {
  ...
  function createReservedRun(overrides: Partial<CreateReservedSubagentRunInput> = {}) {
    ...
    const modelRuntimeSnapshot = overrides.modelRuntimeSnapshot ?? createAmbientModelRuntimeSnapshot("example/model-id", createdAt);
  ...
  it("creates a reserved subagent run and spawn-edge linkage", () => {
    const modelRuntimeSnapshot = createAmbientModelRuntimeSnapshot("example/model-id", now);
