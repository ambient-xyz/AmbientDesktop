...
describe("ProjectStore sub-agent foundation settings", () => {
  ...
  it("persists sub-agent batch jobs and exactly-once result ledgers", async () => {
    ...
    try {
      ...
      const initialProgressEvents = store.listSubagentParentMailboxEventsForParentRun("parent-run")
      ...
      const acceptedProgressEvents = store.listSubagentParentMailboxEventsForParentRun("parent-run")
        .filter((event) => event.type === "subagent.batch_progress");
      ...
      expect(store.listSubagentParentMailboxEventsForParentRun("parent-run")
        .filter((event) => event.type === "subagent.batch_progress")
        .map((event) => ({
          id: event.id,
      ...
      expect(reopened.listSubagentParentMailboxEventsForParentRun("parent-run")
        .filter((event) => event.type === "subagent.batch_progress")
        .map((event) => ({
          id: event.id,
          acceptedReportCount: (event.payload as { summary?: { acceptedReportCount?: number } }).summary?.acceptedReportCount,
  ...
  it("archives cap-exceeded child threads before the cleanup window without touching protected children", async () => {
    ...
    try {
      ...
      expect(cleanup.plan.decisions).toEqual(expect.arrayContaining([
      ...
      expect(store.listSubagentRunEvents(oldest.id)).toEqual(expect.arrayContaining([
        expect.objectContaining({
  ...
  it("skips retention cleanup mutations while ambient.subagents is disabled", async () => {
    ...
    try {
      ...
      expect(store.listSubagentRunEvents(run.id)).not.toEqual(expect.arrayContaining([
        expect.objectContaining({ type: "subagent.retention_archived" }),
      ]));
    } finally {
      store.close();
    }
  ...
  it("archives only retention-eligible child threads and records cleanup audit events", async () => {
    ...
    try {
      ...
      expect(store.listSubagentRunEvents(eligible.id)).toEqual(expect.arrayContaining([
        expect.objectContaining({
          type: "subagent.retention_archived",
          preview: expect.objectContaining({
      ...
      expect(store.getThread(protectedRun.childThreadId).archivedAt).toBeUndefined();
      expect(store.getSubagentRetentionPlan({
        now: "2026-06-05T00:20:00.000Z",
        cleanupWindowMs: 60_000,
      }).decisions).toEqual(expect.arrayContaining([
        expect.objectContaining({
          runId: eligible.id,
          action: "retain",
      ...
      expect(restartSummary.issues).not.toEqual(expect.arrayContaining([
        expect.objectContaining({
  ...
  it("cascades a stopped parent run across dependent child runs, wait barriers, and pending mailbox work", async () => {
    ...
    try {
      ...
      expect(store.listSubagentRunEvents(required.id)).toEqual(expect.arrayContaining([
        expect.objectContaining({
