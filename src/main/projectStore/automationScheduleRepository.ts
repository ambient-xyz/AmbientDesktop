...
export type { ProjectStoreAutomationScheduleRepositoryDeps } from "./automationScheduleTargetResolver";

export class ProjectStoreAutomationScheduleRepository {
  private readonly targetResolver: ProjectStoreAutomationScheduleTargetResolver;

  constructor(
    private readonly db: Database.Database,
    private readonly deps: ProjectStoreAutomationScheduleRepositoryDeps,
  ) {
    this.targetResolver = new ProjectStoreAutomationScheduleTargetResolver(db, deps);
  }
  ...
  createAutomationSchedule(input: CreateAutomationScheduleInput, nowDate = new Date()): AutomationScheduleSummary[] {
    const targetVersion = this.targetResolver.automationScheduleTargetVersion(input.targetKind, input.targetId, input.targetVersion);
    this.targetResolver.requireAutomationScheduleTarget(input.targetKind, input.targetId, targetVersion ?? undefined);
    const blocker = this.targetResolver.automationScheduleCreationBlockReason(input.targetKind, input.targetId, targetVersion ?? undefined);
    ...
    const createdTargetVersionId = this.targetResolver.automationScheduleCreatedTargetVersionId(input.targetKind, input.targetId, targetVersion ?? undefined);
    const dedicatedThreadId = this.targetResolver.automationScheduleDedicatedThreadId(input.targetKind, input.targetId, targetVersion ?? undefined);
  ...
  updateAutomationSchedule(input: UpdateAutomationScheduleInput, nowDate = new Date()): AutomationScheduleSummary[] {
    ...
    const targetVersion = this.targetResolver.automationScheduleTargetVersion(targetKind, targetId, requestedTargetVersion);
    this.targetResolver.requireAutomationScheduleTarget(targetKind, targetId, targetVersion ?? undefined);
    const blocker = this.targetResolver.automationScheduleCreationBlockReason(targetKind, targetId, targetVersion ?? undefined);
    if (blocker) throw new Error(blocker);
    ...
    const nextRunAt = computeAutomationScheduleNextRunAt({ preset, cronExpression, enabled, now: nowDate });
    const createdTargetVersionId = this.targetResolver.automationScheduleCreatedTargetVersionId(targetKind, targetId, targetVersion ?? undefined);
    const dedicatedThreadId = this.targetResolver.automationScheduleDedicatedThreadId(targetKind, targetId, targetVersion ?? undefined, current.dedicated_thread_id ?? undefined);
  ...
  ensureAutomationScheduleDedicatedThread(scheduleId: string): ThreadSummary {
    ...
    const threadId = this.targetResolver.automationScheduleDedicatedThreadId(row.target_kind, row.target_id, row.target_version ?? undefined);
  ...
  private mapAutomationSchedule = (row: AutomationScheduleRow): AutomationScheduleSummary =>
    mapAutomationScheduleRow(row, this.targetResolver.automationScheduleTargetLabel(row.target_kind, row.target_id, row.target_version ?? undefined));
