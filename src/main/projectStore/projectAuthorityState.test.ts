...
const AUTHORITY_STATE_ROOT_ENV = "AMBIENT_AUTHORITY_STATE_ROOT";
...
describeNative("workspace authority state", () => {
  ...
  it("migrates legacy workspace state once and stops reading the legacy database", async () => {
    ...
    try {
      ...
      const reopenedWorkspace = store.openWorkspace(workspacePath);
      expect(store.getDefaultSettings().permissionMode).toBe("workspace");
      expect(join(reopenedWorkspace.statePath, "state.sqlite")).not.toBe(legacyDbPath);
      expect(reopenedWorkspace.statePath.startsWith(authorityRoot)).toBe(true);
    } finally {
      store.close();
      await rm(root, { recursive: true, force: true });
    }
