...
import type { BrowserServiceUserActionController, BrowserUserActionDetection } from "./browserUserActionController";
import {
...
type BrowserRuntimeChromeTargets = Pick<
  ...
  "connectActivePage" | "createTarget" | "evaluatePage" | "getActiveTabSnapshot" | "navigateActiveTarget" | "waitForPageReady"
>;
...
export function normalizePageContent(content: BrowserPageContent): BrowserPageContent {
  return {
    ...
    links: Array.isArray(content.links)
      ? content.links.filter((link) => link && typeof link.url === "string" && typeof link.text === "string").slice(0, 30)
      : [],
  };
}
