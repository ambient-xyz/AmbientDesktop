...
describe("modelProviderCapabilityProbe", () => {
  ...
  it("keeps Ambient known-provider probes compatible with the default profile", () => {
    ...
    const defaultProfileProbeIds: ModelProviderCapabilityProbeId[] = [
      ...planProbeIdsForKnownAmbientDefault(profile),
    ];
    const eligibility = probeModelProviderCapabilityEligibility({
      template,
      profile,
      report: report({
        ...
        passedProbeIds: defaultProfileProbeIds,
      }),
    });
    ...
    if (profile.supportsVision) {
      expect(eligibility.diagnostics.map((diagnostic) => diagnostic.probeId)).toContain("image_input");
    }
  });
});

function planProbeIdsForKnownAmbientDefault(profile: AmbientModelRuntimeProfile): ModelProviderCapabilityProbeId[] {
  const template = modelProviderInstallTemplateById("ambient-managed");
  if (!template) throw new Error("missing ambient-managed template");
  return [
    ...buildModelProviderCapabilityProbePlan({
