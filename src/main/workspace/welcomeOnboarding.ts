...
export interface WelcomeOnboardingResult {
  ...
  instructionsThread: WelcomeOnboardingThreadSummary;
  coreSetupThread: WelcomeOnboardingThreadSummary;
  pluginSetupThread: WelcomeOnboardingThreadSummary;
  copiedAssets: string[];
}

export interface WelcomeOnboardingThreadSummary {
  id: string;
  title: string;
}
...
interface WelcomeOnboardingProjectStore {
  openWorkspace(workspacePath: string): unknown;
  close(): void;
  listThreads(): WelcomeOnboardingThreadSummary[];
  listMessages(threadId: string): Array<{ id: string; content: string; metadata?: Record<string, unknown> }>;
  archiveThread(threadId: string): void;
  createThread(title: string, workspacePath?: string): WelcomeOnboardingThreadSummary;
  addMessage(input: { threadId: string; role: "assistant"; content: string; metadata: Record<string, unknown> }): unknown;
  replaceMessage(messageId: string, content: string, metadata?: Record<string, unknown>): unknown;
  updateThreadTitle(threadId: string, title: string): unknown;
  setThreadPinned(threadId: string, pinned: boolean): unknown;
  markThreadRead(threadId: string): WelcomeOnboardingThreadSummary;
  pruneRedundantEmptyThreads(): void;
  updateThreadWorkspacePath(threadId: string, workspacePath: string): WelcomeOnboardingThreadSummary;
  setLastActiveThreadId(threadId: string): void;
  getThread(threadId: string): WelcomeOnboardingThreadSummary;
}
...
export function ensureWelcomeOnboardingProject(input: {
  ...
  createProjectStore: () => WelcomeOnboardingProjectStore;
  ...
  const welcomeStore = input.createProjectStore();
  try {
    ...
    const seededThreads = new Map<WelcomeOnboardingPageKind, WelcomeOnboardingThreadSummary>();
...
function nudgeWelcomeOnboardingThreadOrder(
  store: WelcomeOnboardingProjectStore,
  workspacePath: string,
  oldestToNewest: WelcomeOnboardingThreadSummary[],
): void {
...
function migrateWelcomeOnboardingSeededThreads(store: WelcomeOnboardingProjectStore): void {
...
): WelcomeOnboardingThreadSummary {
