...
import type { AgentMemoryStarterStatus, AmbientPermissionGrant, LocalRuntimeLeaseRecord, OrchestrationBoard, PermissionAuditEntry, ThreadSummary, WorkspaceState } from "../../shared/types";
...
describe("diagnostics", () => {
  ...
  it("creates a redacted bundle with sqlite, session, and log excerpts", async () => {
    ...
    const payload = await createDiagnosticBundle(
      {
        ...
        getAgentMemoryStarterStatus: () => sampleAgentMemoryStarterStatus("2026-04-29T00:00:00.000Z"),
    ...
    expect(payload.bundle.agentMemory.starterStatus).toMatchObject({
      schemaVersion: "ambient-agent-memory-starter-status-v1",
      state: "needs_repair",
      assets: {
        model: { state: "present", artifactId: "embeddinggemma-300m" },
        runtime: { state: "missing", artifactId: "llama.cpp-darwin-arm64" },
      },
      runtime: { state: "stopped" },
      nextActions: ["repair", "open_logs"],
    });
    expect(payload.bundle.summary.agentMemoryStarter).toMatchObject({
      schemaVersion: "ambient-agent-memory-starter-status-v1",
      state: "needs_repair",
...
function sampleAgentMemoryStarterStatus(checkedAt: string): AgentMemoryStarterStatus {
  return {
    schemaVersion: "ambient-agent-memory-starter-status-v1",
    checkedAt,
    ...
    assets: {
      ...
      runtime: {
        ...
        receiptPath: "/tmp/ambient-memory/runtime.receipt.json",
    ...
    nativePreflight: {
      ...
      coreModuleConfigured: true,
      status: "healthy",
      message: "TencentDB Agent Memory native dependencies are available.",
      dependencies: [],
      errors: [],
    },
    blockers: [{
