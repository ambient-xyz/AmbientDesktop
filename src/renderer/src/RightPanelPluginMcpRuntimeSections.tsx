...
import { mcpContainerRuntimeDetailRows, mcpDefaultCapabilityInstallActionState } from "./pluginUiModel";
...
export type RightPanelMcpRuntimePanelActionState = {
  visible: boolean;
  disabled: boolean;
  title?: string;
  label: string;
...
export function RightPanelMcpRuntimeStatusSection({
  runtimeStatus,
  runtimeToneClass,
  runtimeLabel,
  runtimeBusy,
  ...
  return (
    <section className={`plugin-row mcp-runtime-status-row tone-${runtimeToneClass}`}>
        ...
        <div className="plugin-row-actions">
          ...
          {runtimeStatus?.installPlan?.primaryAction && (
            <>
              ...
              <button type="button" className="panel-button mini" disabled={runtimeLaunchBusy} onClick={onLaunchInstaller}>
                {runtimeLaunchBusy
      ...
      <p>
        {runtimeError ?? runtimeStatus?.message ?? "Ambient checks ToolHive plus Docker or Podman before installing isolated MCP plugins."}
      </p>
      ...
      {runtimeStatus && (
        <>
          <div className="plugin-badges">
            {mcpContainerRuntimeDetailRows(runtimeStatus).map((row) => (
          ...
          {runtimeStatus.installPlan && (
            <div className="plugin-note-list">
              ...
              {runtimeStatus.setup.promptSuppressed && <span>Prompt paused: {formatTaskState(runtimeStatus.setup.reason)}</span>}
              {setupResumeRows.map((row) => (
                <span key={row}>{row}</span>
              ))}
            </div>
          )}
          <RightPanelMcpDefaultCapabilityRows
          ...
          <div className="plugin-note-list">
            {runtimeStatus.hosts.map((host) => (
              <span key={host.kind}>
                {formatTaskState(host.kind)}: {formatTaskState(host.status)}
                {host.version ? ` ${host.version}` : ""}. {host.message}
              </span>
            ))}
          </div>
        </>
...
function RightPanelMcpDefaultCapabilityRows({
  runtimeStatus,
  mcpServerBusy,
  onInstallDefaultCapability,
}: {
  ...
  if (runtimeStatus.defaultCapabilities.length === 0) return null;
...
export function RightPanelManagedDevServersSection({
  managedDevServers,
  ...
  return (
    <section className="plugin-row">
      ...
      <p>
        Dev-server-shaped commands resolve after readiness and continue here as managed background processes instead of blocking the agent
        loop.
      </p>
