import type { ProjectBoardCard, ProjectBoardCardSplitOutcomeStatus, ProjectBoardExecutionArtifact, ProjectBoardPlanningSnapshot, ProjectBoardPmReviewReport, ProjectBoardProofDecisionAction, ProjectBoardSource, ProjectBoardSplitDecisionAction, ProjectBoardSummary, ProjectBoardSynthesisProposal, ProjectBoardSynthesisRun, ProjectSummary } from "../../shared/projectBoardTypes";
...
export { projectBoardDecisionImpactPreview };
export {
...
export { projectBoardDecisionQueue };
export type {
...
export { projectBoardProofEvidenceModel, projectBoardProofInspectionNavigationModel, projectBoardTaskActionEvidenceFromProof };
...
export type ProjectBoardTabId = "overview" | "board" | "map" | "proof" | "integration" | "charter" | "decisions" | "draft_inbox" | "history";
...
export type {
  ProjectBoardCandidateClarificationItem,
...
export function projectBoardActionState(project: ProjectSummary, activeProjectPath: string, busy = false, boardOpen = false): ProjectBoardActionState {
  const active = project.path === activeProjectPath;
  if (!project.board) {
    return {
      kind: "open",
      label: busy ? "Building" : "Project Board",
      title: active ? "Open project board setup. Building starts only from the Build Board button." : "Open this project before viewing its board setup",
...
export function projectBoardThreadPlanActionState(hasBoard: boolean, readyPlanCount: number, busy = false): ProjectBoardThreadPlanActionState {
  if (readyPlanCount <= 0) {
    return {
      kind: "no_ready_plan",
...
export function projectBoardPlanningSnapshotTicketizationState(board: ProjectBoardSummary): ProjectBoardPlanningSnapshotTicketizationState {
  ...
  if (ticketizedCount > 0 && draftCandidateCount > 0) {
    return {
      ...
      tone: "warning",
      detail:
        "Existing Local Tasks are protected. Review additive draft cards or staged Pi updates before creating more tasks.",
      runId: stableRun?.id,
      readyCount,
      draftCandidateCount,
      ticketizedCount,
  ...
  if (stableRun) {
    return {
      ...
      tone: "ready",
      detail:
        "Planner output is at a stable checkpoint. Choose this snapshot by creating ready Local Tasks from the Draft Inbox.",
      runId: stableRun.id,
      readyCount,
      draftCandidateCount,
      ticketizedCount,
  ...
  return {
    ...
    tone: "neutral",
    detail:
      "No completed or paused planning snapshot is available yet. Build or pause planning before bulk ticketization.",
    readyCount,
    draftCandidateCount,
    ticketizedCount,
  };
...
function projectBoardLatestStablePlanningSnapshot(board: Pick<ProjectBoardSummary, "synthesisRuns">): ProjectBoardPlanningSnapshot | undefined {
...
export function projectBoardCreateReadyTasksState(board: ProjectBoardSummary, busy = false): ProjectBoardCreateReadyTasksState {
  ...
  if (projectBoardHasActiveSynthesisRun(board)) {
    return {
      ...
      title: "Wait for board planning to finish or pause before creating Local Tasks. Progressive planner output can still add, replace, or protect draft cards until the active run reaches a stable checkpoint.",
      disabled: true,
    };
  }
...
export function projectBoardEmptyMessage(board?: ProjectBoardSummary): string {
  ...
  if (board.status === "draft" && (board.charter?.version ?? 1) > 1) return "Revision draft is active. Review the charter answers, then apply or cancel the revision.";
...
export function projectBoardColumns(cards: ProjectBoardCard[], orchestrationBoard?: { tasks: OrchestrationTask[]; runs: OrchestrationRun[] }): ProjectBoardColumnModel[] {
  ...
  return [
    { id: "blocked", title: "Blocked", tooltip: "Cards that cannot run yet because dependencies, claims, decisions, proof gates, or execution state are blocking them.", cards: cardsForProjectionStatus("blocked") },
    { id: "ready", title: "Ready", tooltip: "Approved Local Task cards that are eligible to prepare or dispatch when workflow and collaboration checks allow it.", cards: cardsForProjectionStatus("ready") },
    { id: "in_progress", title: "In Progress", tooltip: "Cards with active or recently prepared execution work in progress.", cards: cardsForProjectionStatus("in_progress") },
    { id: "review", title: "Review", tooltip: "Cards waiting for PM review of implementation output, proof, or follow-up decisions.", cards: cardsForProjectionStatus("review") },
    { id: "done", title: "Done", tooltip: "Cards accepted as complete or intentionally covered without more execution.", cards: cardsForProjectionStatus("done") },
  ];
}

export function projectBoardTabs(board: ProjectBoardSummary, orchestrationBoard?: { tasks: OrchestrationTask[]; runs: OrchestrationRun[] }): ProjectBoardTabModel[] {
  ...
  return [
    ...
    { id: "map", label: "Map", count: dependencyIssueCount || dependencyHealth.rows.filter((row) => row.blockedBy.length || row.unblocks.length).length },
...
export function defaultProjectBoardTab(board: ProjectBoardSummary): ProjectBoardTabId {
  ...
  if (projectBoardDecisionQueue(board).actionCount > 0 || (board.proposals ?? []).some((proposal) => proposal.status === "pending")) return "decisions";
  const executableCards = board.cards.filter((card) => Boolean(card.orchestrationTaskId));
...
export function projectBoardPhaseGroups(cards: ProjectBoardCard[], criticalCardIds: Set<string> = new Set()): ProjectBoardPhaseGroup[] {
  ...
  return [...groups.entries()]
    .map(([phase, phaseCards]) => {
      ...
      const blockedCount = phaseCards.filter((card, index) => !projections[index].suppressBlockers && (card.blockedBy.length > 0 || projections[index].visualStatus === "blocked")).length;
...
function projectBoardPhaseTone(
  ...
  if (cards.length > 0 && cards.every((card) => projectBoardCanonicalCardProjection(card).tone === "done" || card.candidateStatus === "evidence")) return "done";
...
export function projectBoardResetImpact(board: ProjectBoardSummary): ProjectBoardResetImpact {
  ...
  const deleted = [
    { label: "Cards", value: board.cards.length, detail: `${draftCards} draft candidate${draftCards === 1 ? "" : "s"}, ${executableCards} executable or linked card${executableCards === 1 ? "" : "s"}.` },
    ...
    { label: "Progress runs", value: synthesisRunCount, detail: "Build Board, Revise Board, retry, and Add Cards synthesis progress records." },
    { label: "Proof/handoff artifacts", value: executionArtifactCount, detail: "Board-owned execution proof, handoff, and follow-up projection records." },
  ...
  const preserved = [
    ...
    linkedTasks > 0 ? `${linkedTasks} existing Local Task${linkedTasks === 1 ? "" : "s"}; they will no longer be attached to this board.` : "Existing Local Task history.",
  ];
  return {
    deleted,
    preserved,
