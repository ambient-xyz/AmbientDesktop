...
import {
  ...
  provider,
} from "./AppConversationMessages.testSupport";
...
describe("AppConversationMessages", () => {
  ...
  it("renders automatic continuation status with the active goal turn count", () => {
    ...
    expect(markup).toContain("automatic goal continuation turn.");
    expect(markup).toContain("Turn 4.");
  });

  it("renders scheduled wake status without goal wording when no goal is active", () => {
    const markup = renderToStaticMarkup(
      <AppConversationMessages
        {...baseProps({
          provider: provider({ hasApiKey: true }),
    ...
    expect(markup).toContain('data-runtime-status-kind="thread-wake"');
    expect(markup).toContain("Scheduled wake");
    expect(markup).toContain("scheduled wake continuation.");
    expect(markup).not.toContain("Continuing goal");
  });
