export function packageJson() {
  return {
    scripts: {
      ...
      "test:subagents:release-gate:unit":
        "pnpm exec vitest run scripts/subagent-release-gate.test.mjs scripts/subagent-release-gate-live-desktop-dogfood.test.mjs scripts/subagent-release-gate-live-confidence.test.mjs scripts/subagent-release-gate-maturity-history.test.mjs scripts/subagent-release-gate-package-scripts.test.mjs",
      ...
      "test:subagents:live-confidence:unit":
        "pnpm exec vitest run scripts/harness-runtime.test.mjs scripts/subagent-live-confidence-plan.test.mjs scripts/subagent-live-confidence-evidence-classification.test.mjs scripts/subagent-live-confidence-blockers.test.mjs scripts/subagent-live-confidence-proof-validators.test.mjs",
...
export function files() {
  return {
    ...
    subagentReleaseGateRunnerTest:
      "passes the deterministic gate with a skipped-live advisory summarizes required-live release gate runs as append-only history rows fails when a required package script is missing",
    ...
    subagentDesktopDogfoodLifecycleRuntimeInspectors:
      "subagentDesktopDogfoodLifecycleRuntimeInspectors inspectLifecycleEdgeVisibility inspectParentStopCascadeVisibility inspectLocalRuntimeOwnership localRuntimeOwnership parentStopCascadeVisibility untrackedRuntimeVisible untrackedStopDisabledVisible untrackedRestartDisabledVisible untrackedForceUnavailableVisible untrackedExternalStopGuidanceVisible stopDisabledVisible affectedSubagentVisible Timeout edge worker Continue with partial Partial approved Retry edge worker Retry requested Child detached Parent-stop required worker Parent cancellation requested 2 pending mailbox events cancelled horizontalOverflowFree",
    ...
    projectStoreSubagentFoundationTest:
      'creates a child thread and run metadata only when ambient.subagents is enabled cascades a stopped parent run across dependent child runs, wait barriers, and pending mailbox work persists explicit quorum thresholds on wait barriers persists parent mailbox parent-message anchors for pre-run spawn failures rejects persisted child runtime and parent mailbox events without exact child attribution parentMessageId: "parent-message" repairs missing and mismatched spawn edges while pruning dangling edges subagent.closed subagent.lifecycle_closed archives cap-exceeded child threads before the cleanup window without touching protected children archives only retention-eligible child threads and records cleanup audit events persists sub-agent batch jobs and exactly-once result ledgers persists Settings-installed model providers and feeds the runtime catalog without secrets',
    subagentPiTools:
    ...
    subagentPiToolsTest:
      "records pre-run spawn failures for local runtime launch preflight denials records pre-run spawn failures for local runtime capacity preflight denials parentMessageId: assistant.id describes direct parent spawn and wait semantics for explicit delegation requests refuses to close actively executing children before releasing capacity closes needs-attention children as abandoned work without deleting history capacityLeaseSnapshot.status transcript and artifacts are retained rejects scheduled spawn requests before creating a live child thread rejects active worker worktrees that are not persisted on the child thread",
    subagentPiToolsLaunchToolScopeTest:
      "reports non-interactive approval-unavailable launch denials to the parent mailbox rejects unknown exact built-in child tools before reserving a child run accepts surfaced extension tools registered in the launch catalog rejects unavailable surfaced extension tools before reserving a child run records visible failed children for Pi-visible connector tools without child-safe bridges records exact source-level tool scope requests in launch snapshots rejects secret-shaped source-level tool ids before reserving a child run rejects broad connector and direct MCP source ids before reserving a child run records a visible failed child when non-callable sources request Pi visibility rejects mistyped tool-scope categories instead of falling back to defaults",
