...
const outputRoot = resolve(process.env.AMBIENT_KANBAN_MARKDOWN_REGEX_PERMISSION_OUT_DIR || join(tmpdir(), "ambient-kanban-markdown-regex-proof-permission-gmi"));
const runStamp = new Date().toISOString().replace(/[:.]/g, "-");
const runRoot = join(outputRoot, "runs", runStamp);
const workspace = join(runRoot, "workspace");
const userData = join(runRoot, "userData");
const reportPath = resolve(process.env.AMBIENT_KANBAN_MARKDOWN_REGEX_PERMISSION_OUT || join(outputRoot, "latest.json"));
const boardScreenshotPath = join(runRoot, "phase5-markdown-regex-board.png");
const proofScreenshotPath = join(runRoot, "phase5-markdown-regex-proof.png");
const cdpPort = Number(process.env.AMBIENT_KANBAN_MARKDOWN_REGEX_PERMISSION_CDP_PORT || 0) || (await availablePort());
...
const sourceUserData = process.env.AMBIENT_KANBAN_MARKDOWN_REGEX_PERMISSION_SNAPSHOT_USER_DATA || process.env.AMBIENT_E2E_USER_DATA || "";
const exactRegexProofCommand =
  "node --input-type=module -e 'import { readFileSync } from \"node:fs\"; const report = readFileSync(\"link-check-report.md\", \"utf8\"); if (!/## Link Check Summary/.test(report)) throw new Error(\"missing markdown summary heading\"); console.log(\"regex proof ok\");'";

const cardSpec = {
  key: "markdown-regex-proof",
  title: `Phase 5 Gate A markdown regex proof ${runStamp}`,
  expectedFiles: ["docs/index.md", "docs/alpha.md", "docs/beta.md", "check-links.mjs", "tests/verify-link-checker.mjs", "link-check-report.json", "link-check-report.md"],
};
...
try {
  if (!sourceWorkspace || !existsSync(sourceWorkspace)) {
    throw new Error("Set AMBIENT_KANBAN_MARKDOWN_REGEX_PERMISSION_SNAPSHOT_WORKSPACE to a local snapshot workspace directory.");
  }
  if (!existsSync(keyFile) && !process.env.GMI_CLOUD_API_KEY && !process.env.GMI_API_KEY) {
    throw new Error("Set GMI_CLOUD_API_KEY_FILE, GMI_CLOUD_API_KEY, or GMI_API_KEY before running the GMI Markdown regex permission gate.");
  }
  ...
  const regexReport = await assertMarkdownRegexReport(result.run.workspacePath);
  const permissionAudit = await assertPermissionAudit();
  const visiblePermissionDialog = await currentPermissionDialogText();
  if (visiblePermissionDialog.includes("/##") || visiblePermissionDialog.includes("Link Check Summary")) {
    throw new Error(`A visible permission dialog referenced the Markdown regex proof command: ${visiblePermissionDialog}`);
  }
  ...
  const report = {
    ...
    assertions: [
      ...
      "Permission audit recorded the regex command as an allowed workspace-scoped proof command",
      "No outside-workspace permission prompt or visible permission dialog was produced for the /## regex",
    ],
  };
...
async function sanitizeTempWorkspace() {
  ...
  await rm(join(workspace, "docs", "phase5-markdown-regex-proof"), { recursive: true, force: true });
...
agent:
  ...
  extra_instructions: |
    This is a bounded Phase 5 semantic permission gate. Do not inspect, print, summarize, or copy API keys, secret files, local-secrets folders, credentials, browser profiles, or environment values. Do not install packages or use network calls. Keep all writes inside the writable task workspace.
proof_of_work:
  require_tests: true
...
async function createBoardAndCard() {
  ...
  if (!activeProject) throw new Error("No active project available for Markdown regex permission gate.");
  const stateWithBoard = await invoke("createProjectBoard", {
    projectId: activeProject.id,
    title: "Phase 5 Markdown Regex Permission Gate Board",
    summary: "Live Gate A proof that inline /## Markdown regex proof commands do not trigger false outside-workspace prompts.",
  });
  ...
  const card = await createReadyCard(board.id, cardSpec.title, markdownRegexProofDescription(), {
    priority: 1,
    labels: ["phase-5", "semantic-permissions", "markdown-regex", "proof-command", "live-gmi"],
    blockedBy: [],
    acceptanceCriteria: [
      "Create a deterministic markdown fixture set and dependency-free link checker.",
      "Run the exact inline Node proof command containing /## Link Check Summary/.",
      "Write link-check-report.json and link-check-report.md with zero broken links and regex proof evidence.",
    ...
    testPlan: {
      unit: ["Run node --check check-links.mjs.", "Run node tests/verify-link-checker.mjs."],
      integration: [exactRegexProofCommand],
      visual: [],
      manual: [],
    },
  });
...
function markdownRegexProofDescription() {
  return [
    "Create a dependency-free Markdown link-checker proof in this card's writable task workspace. Do not modify the owning project root directly.",
    "",
    "Required files:",
    "- docs/index.md: include '# Link Index', a '## Link Check Summary' heading, and relative links to alpha.md and beta.md.",
    "- docs/alpha.md: include '# Alpha'.",
    "- docs/beta.md: include '# Beta'.",
    "- check-links.mjs: dependency-free Node ESM script exporting checkMarkdownLinks(root). It must scan markdown files under root, resolve relative .md links, and return checkedFiles and brokenLinks.",
    "- tests/verify-link-checker.mjs: dependency-free verifier that imports checkMarkdownLinks, runs it against docs, asserts zero broken links, writes link-check-report.json, and writes link-check-report.md with '## Link Check Summary'. The JSON report must include passed true, regexProof true, checkedFiles, and brokenLinks.",
    "- link-check-report.json and link-check-report.md: generated proof artifacts from the verifier.",
    "",
    "Verification commands to run from the task workspace:",
    "- node --check check-links.mjs",
    "- node tests/verify-link-checker.mjs",
    `- ${exactRegexProofCommand}`,
    "",
    "Exact regex proof requirement:",
    `- Run this exact command without replacing the regex with includes/string matching: ${exactRegexProofCommand}`,
    "- The inline JavaScript regex must remain /## Link Check Summary/ so Ambient can prove the permission policy treats it as proof syntax, not an outside filesystem path.",
    "",
    ...
    "- After writing and verifying files, call task_report_proof with changedFiles exactly including docs/index.md, docs/alpha.md, docs/beta.md, check-links.mjs, tests/verify-link-checker.mjs, link-check-report.json, and link-check-report.md.",
    "- Include all three verification commands in the proof commands, including the exact inline Node regex command.",
...
async function approveRunAndVerifyCard(card, spec) {
  ...
  if (!description.includes("/## Link Check Summary/")) {
    throw new Error(`${spec.title} task description lost the inline Markdown regex proof command.`);
  }
...
function expectedNeedleForFile(file) {
  if (file === "docs/index.md") return "## Link Check Summary";
  if (file === "docs/alpha.md") return "# Alpha";
  if (file === "docs/beta.md") return "# Beta";
  if (file === "check-links.mjs") return "checkMarkdownLinks";
  if (file === "tests/verify-link-checker.mjs") return "link-check-report.json";
  if (file === "link-check-report.json") return '"regexProof"';
  if (file === "link-check-report.md") return "## Link Check Summary";
  return "";
}

async function assertMarkdownRegexReport(runWorkspacePath) {
  const markdown = await assertAbsoluteFile(join(runWorkspacePath, "link-check-report.md"), "## Link Check Summary");
  if (!/## Link Check Summary/.test(markdown)) throw new Error("Markdown proof report did not satisfy /## Link Check Summary/.");
  const text = await assertAbsoluteFile(join(runWorkspacePath, "link-check-report.json"), '"regexProof"');
  const report = JSON.parse(text);
  if (report.passed !== true) throw new Error(`Expected report.passed true: ${text}`);
  if (report.regexProof !== true) throw new Error(`Expected report.regexProof true: ${text}`);
  ...
  const checkedFiles = Array.isArray(report.checkedFiles) ? report.checkedFiles.map(checkedFileLabel).filter(Boolean) : [];
  for (const file of ["index.md", "alpha.md", "beta.md"]) {
    if (!checkedFiles.some((checked) => checked.endsWith(file))) throw new Error(`Report did not include checked file ${file}. checkedFiles=${checkedFiles.join(", ")}`);
  }
  return { passed: report.passed, regexProof: report.regexProof, checkedFiles, brokenLinks };
}

function checkedFileLabel(item) {
  if (typeof item === "string") return item;
  if (item && typeof item === "object") {
    for (const key of ["file", "path", "name"]) {
...
async function assertPermissionAudit() {
  ...
  const regexEntries = audit.filter((entry) => entry.toolName === "bash" && String(entry.detail || "").includes("/## Link Check Summary/"));
  if (!regexEntries.length) throw new Error("Permission audit did not record the inline /## regex proof command.");
  const allowedProof = regexEntries.find((entry) => entry.decision === "allowed" && entry.risk === "workspace-command" && String(entry.reason || "").includes("proof command"));
  if (!allowedProof) {
    throw new Error(`No allowed proof-command audit entry found for the /## regex. Entries=${JSON.stringify(regexEntries)}`);
  }
  const falseOutsidePrompt = audit.find(
    (entry) => entry.risk === "outside-workspace" && (String(entry.detail || "").includes("/## Link Check Summary/") || String(entry.detail || "").includes("Outside path: /##")),
  );
  if (falseOutsidePrompt) throw new Error(`False outside-workspace prompt for inline Markdown regex: ${JSON.stringify(falseOutsidePrompt)}`);
  return {
    regexCommandAuditCount: regexEntries.length,
    allowedProofReason: allowedProof.reason,
    allowedProofRisk: allowedProof.risk,
    outsideWorkspaceRegexPromptCount: audit.filter((entry) => entry.risk === "outside-workspace" && String(entry.detail || "").includes("/##")).length,
  };
}
...
function classifyHarnessFailure(error) {
  ...
  if (/Local Task|proof|permission|outside-workspace|regex|link-check|project-board|card|board|verification/i.test(message)) return "product";
  return "test harness";
}
