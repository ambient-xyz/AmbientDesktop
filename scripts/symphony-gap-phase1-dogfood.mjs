...
const resultsDir = join(repoRoot, "test-results", "symphony-gap-phase1-dogfood");
const scratchRoot = await mkdtemp(join(tmpdir(), "ambient-symphony-gap-phase1-"));
...
const composerPrompt = "Compare three weekend trip options and recommend one.";
const reducerMetric = "Reducer must compare every child result, cite each option, and state a winner.";
...
try {
  ...
  if (process.env.AMBIENT_SYMPHONY_GAP_PHASE1_KEEP_SCRATCH !== "1") {
    await rm(scratchRoot, { recursive: true, force: true });
  } else {
    process.stdout.write(`Symphony gap Phase 1 dogfood scratch retained at ${scratchRoot}\n`);
  }
}

process.exit(exitCode);

async function runPhase1Dogfood() {
  ...
  try {
    ...
    await setTextAreaValue(cdp, ".composer-input-wrap textarea", composerPrompt);
    await waitForTextareaValue(cdp, ".composer-input-wrap textarea", composerPrompt);
    await clickElement(cdp, "button[data-ui-required-action='composer-send']");
    await waitForText(cdp, "Complete required reducer success metric before confirming the launch card.");
    await expectNoText(cdp, "Symphony workflow launch sent to Ambient.");
    ...
    await waitForTextareaValue(cdp, ".composer-input-wrap textarea", composerPrompt);
    await waitFor(cdp, () => Boolean(document.querySelector(".symphony-confirmation-state.ready")));
    await clickElement(cdp, "button[data-ui-required-action='composer-send']");
    const persistedUserMessage = await waitForPersistedSymphonyUserMessage();
    await expectNoText(cdp, "Prompt sent to Ambient.");

    artifacts.desktopScreenshot = await writeScreenshot(cdp, "symphony-gap-phase1-desktop.png");
    artifacts.accessibilitySnapshot = await writeAccessibilitySnapshot(cdp, "symphony-gap-phase1-accessibility.json");
    await writeReport({
      schemaVersion: "ambient-symphony-gap-phase1-dogfood-v1",
      status: "passed",
      ...
      headful: true,
      cdpPort: port,
      scenarios: ["symphony_gap_phase1_toggle_to_intent"],
      checks: {
        incompleteLaunchBlocked: true,
    ...
    if (cdp) {
      try {
        artifacts.failureScreenshot = await writeScreenshot(cdp, "symphony-gap-phase1-failure.png");
      } catch {
...
async function waitForPersistedSymphonyUserMessage() {
  ...
  while (Date.now() - started < 30_000) {
    ...
    const match = lastRows.find((row) => {
      ...
      return row.content === composerPrompt &&
...
function dogfoodStateDbPath() {
  ...
  const authorityDbPath = join(
    userDataPath,
    "authority-state",
    "workspaces",
    authorityWorkspaceDirectoryName(workspacePath),
    "state.sqlite",
  );
