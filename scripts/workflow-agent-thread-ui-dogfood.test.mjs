...
describe("Workflow Agent V3 UI dogfood harness", () => {
  it("stays syntactically valid as a standalone Electron harness", async () => {
    await expect(execFileAsync(process.execPath, ["--check", new URL("./workflow-agent-thread-ui-dogfood.mjs", import.meta.url).pathname])).resolves.toBeTruthy();
    await expect(execFileAsync(process.execPath, ["--check", new URL("./workflow-agent-thread-ui-dogfood-matrix.mjs", import.meta.url).pathname])).resolves.toBeTruthy();
  });

  it("covers the live narrow-split Workflow Agent lifecycle", async () => {
    const source = await readFile(new URL("./workflow-agent-thread-ui-dogfood.mjs", import.meta.url), "utf8");
  ...
  it("wires validated GWS snapshots into Desktop launches without mutating source credentials", async () => {
    ...
    try {
      ...
      const snapshotBinary = join(snapshot, "userData", "tools", "google-workspace-cli", "v0.22.3", `${process.platform}-${process.arch}`, "gws");
  ...
  it("discovers bounded home shared snapshots when snapshot mode is requested", async () => {
    ...
    try {
      ...
      expect(ready.candidateRoots).toEqual(expect.arrayContaining([
        expect.objectContaining({
  ...
  it("requires inspectable final workflow output for UI dogfood product gates", () => {
    const baseDetail = {
      events: [
        ...
        workflowEvent({ type: "ambient.call.end", message: "study-card" }),
      ],
  ...
  it("treats connector reads, pagination counts, and connector writes as product gates", () => {
    ...
    const expectConfig = {
      ...
      forbiddenConnectorMessages: ["google.gmail.createDraft", "google.gmail.updateDraft", "google.gmail.deleteDraft", "google.gmail.sendDraft"],
      requiredFinalOutputTerms: ["read-only"],
      requiredFinalOutputAnyTerms: [["category", "bucket"], ["count", "messages", "threads"], ["example", "evidence", "provenance"], ["partial", "coverage", "observed"]],
    };
  ...
  it("enforces Gmail 1000 metadata-first connector budgets and review gates", () => {
    ...
    const detail = {
      ...
      checkpoints: [{ key: "gmailMetadataCoverage", valuePreview: JSON.stringify({ searchPages: 10, metadataOnly: true, readOnly: true }) }],
    };
    const expectConfig = {
      minModelCalls: 1,
      maxModelCalls: 50,
      ...
      forbiddenConnectorMessages: ["google.gmail.readThread", "google.gmail.createDraft", "google.gmail.updateDraft", "google.gmail.deleteDraft", "google.gmail.sendDraft"],
      requiredFinalOutputTerms: ["metadata-only", "read-only"],
      requiredFinalOutputAnyTerms: [["category", "bucket"], ["count", "messages", "threads"], ["example", "evidence", "provenance"], ["follow-up", "detail-read", "approval", "review"]],
    };
    ...
    expect(
      assertWorkflowUiDogfoodEvidence(detail, {
        ...
        expectConfig: {
          ...
          requiredFinalOutputAnyTerms: [["theme", "category", "bucket"], ["messages", "threads"], ["coverage", "insufficient", "partial"]],
        },
      }),
    ).toMatchObject({
      passed: true,
  ...
  it("enforces Google transcript connector provenance and long-context routing", () => {
    const detail = {
      ...
      modelCalls: [{ status: "succeeded" }],
      checkpoints: [{ key: "googleTranscriptCoverage", valuePreview: JSON.stringify({ calendarEvents: 10, driveCandidates: 4, transcriptReads: 3, readOnly: true }) }],
    };
    const expectConfig = {
      minModelCalls: 1,
      maxModelCalls: 3,
      ...
      forbiddenConnectorMessages: ["google.calendar.createEvent", "google.drive.createFile", "google.drive.updateFile", "google.drive.trashFile"],
      ...
      assertWorkflowUiDogfoodEvidence(
        {
          ...
          events: [
            ...detail.events,
            workflowEvent({ type: "desktop-tool.end", message: "local_file_read", seq: 2 }),
          ],
        },
        {
      ...
      assertWorkflowUiDogfoodEvidence(
        {
          ...
          events: [
            ...detail.events,
            workflowEvent({ type: "desktop-tool.end", message: "local_directory_list", seq: 2 }),
          ],
        },
        {
      ...
      assertWorkflowUiDogfoodEvidence(
        {
          ...
          events: [
            ...detail.events,
            workflowEvent({ type: "desktop-tool.end", message: "file_read", seq: 22 }),
          ],
        },
        {
  ...
  it("keeps bounded scenarios for model, local-file, and public-browser workflow shapes", async () => {
    ...
    expect(source).toContain('forbiddenToolMessages: ["local_file_read", "file_read", "file_write", "bash", "browser_search", "browser_nav", "browser_content", "google_workspace_call"]');
    ...
    expect(source).toContain('forbiddenModuleFragments: ["browser", "gmail", "google-workspace", "current-data", "movie-night", "visual-analysis"]');
  ...
  it("requires recovery provenance for flaky browser/search dogfood gates", () => {
    const detail = {
      events: [
        ...
        workflowEvent({ type: "workflow.recovery.skipped_item", message: "fetch sources", data: { action: "skip_item", itemKey: "bad-source" } }),
      ...
      modelCalls: [{ status: "succeeded" }],
      checkpoints: [{ key: "partialCoverage", valuePreview: JSON.stringify({ skipped: ["bad-source"], retained: ["example-source", "iana-source"] }) }],
    };
