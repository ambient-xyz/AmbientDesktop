...
import { V3_HOTSPOTS } from "./simplification-v3-scorecard.mjs";

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..");

export const V3_RENDERER_BASELINE_HOTSPOT_IDS = [
  "right-panel-shell",
  "app-shell",
...
export const V3_RENDERER_BASELINE_GROUPS = [
  {
    ...
    parityTarget: "Right panel shell, composer-adjacent controls, provider dialogs, permission prompt geometry, and extracted pane boundaries.",
  },
  {
    id: "app-shell",
    hotspotId: "app-shell",
  ...
  },
  {
    ...
    parityTarget: "Automation workspace list/edit flows before moving workspace JSX.",
  },
  {
    ...
    parityTarget: "Board columns, dense cards, long generated content, draft detail, PM-review inspector geometry, and evidence/action panels.",
  },
];

export function buildV3RendererBaselineManifest(hotspots = V3_HOTSPOTS) {
  const byId = new Map(hotspots.map((hotspot) => [hotspot.id, hotspot]));
  const groups = V3_RENDERER_BASELINE_GROUPS.map((group) => {
    const hotspot = byId.get(group.hotspotId);
    const resultsDir = `test-results/simplification-v3-baselines/${group.id}`;
    const fixtureRoot = `test-results/simplification-v3-baselines-fixture/${group.id}`;
    return {
      ...group,
      owner: hotspot?.owner,
      file: hotspot?.file,
  ...
  return {
    generatedFrom: "simplification V3 renderer baseline manifest",
    requiredHotspotIds: V3_RENDERER_BASELINE_HOTSPOT_IDS,
    groupCount: groups.length,
    groups,
  };
}

export function validateV3RendererBaselineManifest(manifest, options = {}) {
  const issues = [];
  ...
  const hotspotIds = new Set(V3_HOTSPOTS.map((hotspot) => hotspot.id));
  ...
  for (const group of manifest.groups) {
    ...
    if (!group.outputs?.resultsDir?.startsWith("test-results/simplification-v3-baselines/")) {
      issues.push(`${group.id}: results must stay under test-results/simplification-v3-baselines`);
    }
    if (!group.outputs?.fixtureRoot?.startsWith("test-results/simplification-v3-baselines-fixture/")) {
      issues.push(`${group.id}: fixture root must stay under test-results/simplification-v3-baselines-fixture`);
    }
...
export function renderV3RendererBaselinesMarkdown(manifest) {
  const lines = [
    "# Simplification V3 Renderer Baselines",
    "",
...
function usage() {
  return [
    "Usage: node scripts/simplification-v3-renderer-baselines.mjs [--json] [--check]",
    "",
    "Prints the UI-model baseline capture commands for V3 renderer hotspots and checks that every scenario still exists.",
  ].join("\n");
}
...
function main() {
  ...
  const validation = validateV3RendererBaselineManifest(manifest);
