...
describe("project-board kanban full-cut gate", () => {
  ...
  it("fails when a phase loses its Progress section", async () => {
    ...
    expect(report.issues).toContain("Phase 1 must keep separate Implementation Slice, Progress, and Gate Scenario sections with Gate A and B.");
  });

  it("fails when a documented GMI gate script is removed", async () => {
    const facts = await readKanbanFullCutGateFacts(repoRoot);
    delete facts.packageJson.scripts["test:project-board-kanban-contrast-native-task-actions:gmi"];

    const report = evaluateKanbanFullCutGateFacts(facts);

    expect(report.status).toBe("failed");
    expect(report.issues).toContain("package.json must expose every reviewed kanban GMI gate script with its deterministic command.");
  });
  ...
  it("fails when the native task prompt falls back to the old broad availability wording", async () => {
    ...
    facts.taskToolsSource = facts.taskToolsSource
      ...
      .replace("Fallback path: use a fenced ```task_actions JSON array only when native task tools are unavailable", "Fallback path omitted.");

    const report = evaluateKanbanFullCutGateFacts(facts);

    expect(report.status).toBe("failed");
    expect(report.issues).toContain("Phase 7 must keep native task tools as the primary contract and fenced task_actions JSON as fallback-only.");
  });
