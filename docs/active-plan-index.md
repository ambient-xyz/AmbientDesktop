...
Provider-dependent validation follows `Agents.md`: while Use an available supported provider for live validation, use Ambient with Kimi (`AMBIENT_PROVIDER=ambient`, `AMBIENT_LIVE_MODEL=<model-id>`). Treat GMI Cloud as explicit-request or approved-failover inventory only.
