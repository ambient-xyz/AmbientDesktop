...
Provider note, 2026-06-16: Use an available supported provider for live validation. Current live Ambient/Pi validation must use the Ambient provider with Kimi (`AMBIENT_PROVIDER=ambient` and `AMBIENT_LIVE_MODEL=<model-id>`). Do not use GLM 5.1 or GMI Cloud for operative simplification validation unless a future plan explicitly supersedes this note.
