...
export interface TdaiStoreInitStatus {
  completed: boolean;
...
export interface TdaiReindexProgress {
  done: number;
  total: number;
  layer: "L1" | "L0";
}

export interface TdaiReindexResult {
...
export class TdaiCore {
  ...
  private storeInitStatus: TdaiStoreInitStatus = {
    completed: false,
    needsReindex: false,
  ...
  getStoreInitStatus(): TdaiStoreInitStatus {
    return { ...this.storeInitStatus };
  }
  ...
  async reindexAllEmbeddings(
    onProgress?: (progress: TdaiReindexProgress) => void,
  ): Promise<TdaiReindexResult> {
    await this.waitForStoreReady();
    if (!this.storeInitStatus.needsReindex) {
      return {
        status: "not_required",
        l1Count: 0,
        l0Count: 0,
        ...(this.storeInitStatus.reindexReason ? { reason: this.storeInitStatus.reindexReason } : {}),
      };
    }
    if (!this.vectorStore) {
      return {
    ...
    if (!this.embeddingService) {
      return {
        ...
        l1Count: 0,
        l0Count: 0,
        reason: "Embedding service is unavailable.",
      };
    }
    try {
      const result = await this.vectorStore.reindexAll(
        (text) => this.embeddingService!.embed(text),
        (done, total, layer) => onProgress?.({ done, total, layer }),
      );
      this.storeInitStatus = {
        ...this.storeInitStatus,
        needsReindex: false,
      };
      delete this.storeInitStatus.reindexReason;
      return {
        status: "complete",
        l1Count: result.l1Count,
        l0Count: result.l0Count,
      };
    } catch (err) {
      return {
        status: "error",
  ...
  private async initStores(): Promise<void> {
    try {
      const stores: StoreInitResult = await initStores(this.cfg, this.dataDir, this.logger);
      this.vectorStore = stores.vectorStore;
      this.embeddingService = stores.embeddingService;
      this.storeInitStatus = {
        completed: true,
        needsReindex: stores.needsReindex,
        vectorStoreAvailable: Boolean(this.vectorStore),
        embeddingServiceAvailable: Boolean(this.embeddingService),
        ...(stores.reindexReason ? { reindexReason: stores.reindexReason } : {}),
      };
      ...
      this.logger.warn(
        `${TAG} Store init failed; recall/dedup degraded: ${message}`,
      );
    }
  }
